...
document.getElementById('forest')!.onclick = () => view(new Vector3(1550, 32, 240));
...
game.scene.onBeforeRenderObservable.add(() => {
  ...
  camera.rotationQuaternion = Quaternion.FromEulerAngles(Number(input('pitch').value) * Math.PI / 180, Number(input('head').value) * Math.PI / 180, 0);
  const intent = game.flight.createXRIntent(poses);
  if (input('fly').checked) game.flight.update(game.engine.getDeltaTime() / 1000, {throttle:0, brake:0, yaw:0, pitch:0}, undefined, intent);
  if (performance.now() - lastStats > 250) {
    document.getElementById('stats')!.textContent = `Thrust: ${(intent.throttle * 100).toFixed(1)}% · Heading: ${intent.direction.asArray().map((x) => x.toFixed(2)).join(', ')} · ${game.engine.getFps().toFixed(0)} FPS · ${game.scene.getActiveMeshes().length} active meshes`;
